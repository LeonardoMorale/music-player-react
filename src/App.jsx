...
class App extends Component {
  ...
  componentDidMount() {
    ...
    console.info('App montada. Canciones cargadas:', songs.length);
    if (songs[0]) {
      this.audioPlayer.src = URL.createObjectURL(songs[0]);
    } else {
      console.error('No hay canciones disponibles para cargar al iniciar');
    }
  ...
  playSong = (id) => {
    ...
    if (songs[id]) {
      console.log('Reproduciendo canción:', songs[id].name);
